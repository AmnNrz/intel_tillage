...
for iter in np.arange(1, 21):
    for trial in np.arange(1, 20):
        print(f"trial: {trial}")
        round_num = trial  # Change this to the desired round number
        ...
        # --- Robust reliability tiers from quantiles (handles ties) ---
        acc = metrics_df['acc'].clip(0, 1)

        quantiles = [0, 1/3, 2/3, 1]
        # Get bin edges; duplicates='drop' collapses identical quantiles
        _, bins = pd.qcut(acc, q=quantiles, retbins=True, duplicates='drop')
        n_intervals = len(bins) - 1  # how many bins we actually have

        if n_intervals <= 1:
            # All accuracies essentially identical -> assign a single tier
            metrics_df['reliability'] = 'medium'
        else:
            # Match labels to the actual number of bins (2 or 3)
            label_choices = {2: ['low', 'high'], 3: ['low', 'medium', 'high']}
            labels = label_choices[n_intervals]
            metrics_df['reliability'] = pd.cut(
                acc, bins=bins, labels=labels, include_lowest=True) # type: ignore
        ...
        def active_cp_first_round(steps=1000, target_coverage=0.9, gamma=5, alpha=4, round=0, save_csv=True):
            ...
            csv_save_path = os.path.join(
                BASE,
                f'sampling_methods_results/active_cp_rounds/active_cp_iter_{iter}_round_{round}.csv')
            if save_csv:
